...
setName(event.target.value)
  ...
  return (
    <article className="flex flex-col p-4 bg-white  max-w-xs justify-center items-center rounded-lg overflow-hidden ">
    
    {loggedIn ? (
      <>
      <h3 className="font-bold">Velkommen tilbage {name}</h3>
      <p>Se dine resultater her</p>
      <button className="cursor-pointer text-white mt-2 bg-rose-500 px-4 py-2 rounded-sm">
      Din side
    </button>
      </>
    ) : (
      ...
      <label className="mt-5 text-center" htmlFor="username">
        Brugernavn
        <input
          className=" flex flex-col bg-slate-200 mt-2 py-1 rounded px-1"
          id="username"
          name="username"
          type="text"
